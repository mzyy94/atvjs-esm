...
function getErrorDoc(message) {
    ...
    if (_.isPlainObject(message)) {
        ...
        if (cfg.status &&
            !cfg.template &&
            defaults$1.templates?.status?.[cfg.status]) {
...
function presentModal(modal) {
    let doc; // assume a document object is passed
    if (_.isString(modal)) {
    ...
    select: {
        ...
        onLinkClick(e) {
            ...
            try {
                ...
                console.warn(`Invalid value for the page options (${hrefOptionsAttribute}=${attr}) in the template.`);
                options = {};
            }
...
function initLibraries(cfg = {}) {
    _.each(configMap, (keys, libName) => {
        ...
        "setOptions" in lib && lib.setOptions(options);
    });
}
